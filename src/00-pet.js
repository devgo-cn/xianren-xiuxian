...
export const PET_CFG = {
   ...
   *
   * ⚠️ v7.10 门槛整体下调（实测反馈：玩家在 200 关附近看到"永远只跳一关"）。
   *    旧的第二档门槛是 3000 —— 那意味着 best < 3000 时可选集合恒为 {0,1}，
   *    掷一万次也只有"不跳"和"跳 1 关"两种结果，"随机档位"在体感上等于没实现。
   *    现在把六档全部压进前 2% 的赛程内（2000 关解锁满档），
   *    玩家开局几十关就能抽到 2 关，几百关能抽到 8 关，
   *    此后再靠【面板/境界】而不是跳关档位提供长期成长。
   */
  SKIP_STEPS: [
    [0, 1],
    [120, 2],
