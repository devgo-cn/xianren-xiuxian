...
/** 第 1 层需要的转生轮数（开局半天一层） */
export const ROUNDS_BASE = 114;   // = Λ（v7.1 每境节奏系数，需求方定 33 天毕业）

/** 后期陡度：v7.1 同形成本下必须为 1 —— 每境【恒定】Λ=114 轮。
 *  旧值 9（18 → 162 轮递增）会让后期境界成本相对收入的形状走偏。 */
export const ESCALATE = 1;
 ...
 *
 * 27.0464 是 v7.1 门禁在【本仓库 H0=1.0055 刻度】下的精确下界值。
 *    取值逻辑（务必看懂再改）：
 *      要求 第54境(天仙) 可达 ≥30000 关 且 第53境 可达 <30000 关
 *      ⟹ Q ∈ [27.0464, 28.8171)
 *      取【下界】= 27.0464：既让 R54 恰好够到 30000，
 *      又让 R53 停在 29449 关（离顶 551 关），门禁缓冲最大。
 *    旧值 28.3 / 28.2444 也在可行区间内，但 R53 距顶仅 175 关，
 *    宠物跳关或离线补课时容易越过，门禁形同虚设。
 *         ⚠️ Q 必须 < 28.8171，否则第 53 境就能推满 30000 关，
 *            玩家不用成天仙也能通关 —— 直接违背需求方「天仙以后才能通关」。
 */
export const Q_REALM_REF = 27.0464;
 ...
 *
 * ⚠️ v7.1 口径：线性 1 + C·MAX_LV = 10.9 倍（不是指数 (1+C)^L ≈19900）。
 *    两者差 1800 倍，直接决定单轮跨度：
 *        指数 → log(19900)/log(1.006) ≈ 1655 关/轮（旧版手感，装备是推关主力）
 *        线性 → log(10.9)/log(1.006)  ≈  399 关/轮（v7.1，境界才是推关主力）
 *    v7.1 全套标定（Q=28.2444 / Λ=114 / 33 天）建立在线性 10.9 之上。
 *    改 00-equip.EQ_CFG 时必须同步这里的常量（有测试断言）。
 */
/* 必须从 EQUIP_MAX_BONUS 派生：历史上本文件独立写了一份公式，
 * 与 00-equip.slotMult 口径漂移（19900 vs 10.9），使境界门槛自洽性偏差 76%。 */
export const EQ_MULT = E.EQUIP_MAX_BONUS;
