...
但改完发现**还是不对**：实测 420x860 上
- 顶部 HUD 占 `y 0~60`
- 战斗横带 `top=92, height=0.56*vh-176` → `y 92~398`（会铺不透明底色）

横带**之上**的纯净天空只剩 `y 60~92`（**32px**）—— 装不下直径 ~90px 的月亮。
`fy=0.135`（center y≈116）虽然避开了 HUD，却被横带拦腰截断，
看起来像"半块月亮挂在战斗区边缘"。

**最终解法：把月亮从 bg 里拆出来，成为独立层 `moonLayer()`，
插在 `battle` 之后。**
这样它既落在横带区域内（不被横带底色压住），又不遮战斗（battle 在它下面）。
位置取 `fx=0.815 / fy=0.105`（860 高屏 center y≈90），
月轮上半在 HUD 之下、下半自然垂在夜空与战斗区交界 —— 就是"右上角挂一盏月亮"。

最终层顺序：

```
bg → battle → moon → aura → burst   (dantian 借 ticker，输出到角色 overlay)
```

> 轮体本身在 sprite 里就是**不透明实心盘 + 极淡外晕**，用 `source-over` 原样绘制。
> 实测剖面是一块干净的亮平台：`±30px` 全是 ~230，`±40px` 骤降到 16 —— 边界利落，
> 说明既是实心、又没有"外圈发虚"的雾感。
...
| 项目 | 结果 |
|---|---|
| 层顺序 | `bg → battle → moon → aura → burst`（dantian 仅借 ticker，输出到角色 overlay） |
| 丹田 overlay 贴合角色框 | ✅ 346×350 严丝合缝 |
| 丹田中心亮度 | ✅ 255，剖面 `0→204→235→255→234→191→0` 单峰 |
| 外圈无旋臂星点 | ✅ max = 0（半径 105px 处全黑） |
| 月亮可见 | ✅ avg = 242.6，30 帧 min 242.0 / max 243.0（稳定不闪）<br>剖面 ±30px 全 ~230、±40px 骤降 16（实心盘，边界利落） |
| 绿闪 | ✅ 40 点 × 40 帧扫描，未发现 |
